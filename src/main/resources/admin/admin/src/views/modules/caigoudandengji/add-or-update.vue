<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="采购单编号" prop="caigoudanbianhao">
            <el-input v-model="ruleForm.caigoudanbianhao" 
                placeholder="采购单编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.caigoudanbianhao" label="采购单编号" prop="caigoudanbianhao">
              <el-input v-model="ruleForm.caigoudanbianhao" 
                placeholder="采购单编号" readonly></el-input>
          </el-form-item>
        </div>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `caigoudandengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.caigoudandengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
