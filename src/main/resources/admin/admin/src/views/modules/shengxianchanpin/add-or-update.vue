<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="保存状态" prop="baocunzhuangtai">
          <el-select :disabled="ro.baocunzhuangtai" v-model="ruleForm.baocunzhuangtai" placeholder="请选择保存状态">
            <el-option
                v-for="(item,index) in baocunzhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="保存状态" prop="baocunzhuangtai">
	      <el-input v-model="ruleForm.baocunzhuangtai"
                placeholder="保存状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="出产日期" prop="chuchanriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.chuchanriqi" 
                type="date"
                :readonly="ro.chuchanriqi"
                placeholder="出产日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.chuchanriqi" label="出产日期" prop="chuchanriqi">
              <el-input v-model="ruleForm.chuchanriqi" 
                placeholder="出产日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="生鲜详情" prop="shengxianxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.shengxianxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shengxianxiangqing" label="生鲜详情" prop="shengxianxiangqing">
                    <span v-html="ruleForm.shengxianxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	tupian : false,
	guige : false,
	baocunzhuangtai : false,
	chuchanriqi : false,
	shengxianxiangqing : false,
	clicktime : false,
	clicknum : false,
	price : false,
	onelimittimes : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.guige = true;
          ...
          if(o=='baocunzhuangtai'){
            this.ruleForm.baocunzhuangtai = obj[o];
	    this.ro.baocunzhuangtai = true;
            continue;
          }
          if(o=='chuchanriqi'){
            this.ruleForm.chuchanriqi = obj[o];
	    this.ro.chuchanriqi = true;
            continue;
          }
          if(o=='shengxianxiangqing'){
            this.ruleForm.shengxianxiangqing = obj[o];
	    this.ro.shengxianxiangqing = true;
            continue;
          }
          if(o=='clicktime'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.baocunzhuangtaiOptions = "冷冻,冷藏,冰鲜,活鲜".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shengxianchanpin/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.shengxianxiangqing = this.ruleForm.shengxianxiangqing.replace(reg,'../../../springbootdvzk6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shengxianchanpin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shengxianchanpinCrossAddOrUpdateFlag = false;
						 this.parent.search();
